...
def insert_into_courses(course_name, course_description,teacher_id):
    sql_insert = """INSERT INTO courses (course, course_description, teacher_id)
    VALUES (:course_name, :course_description, :teacher_id)"""
    db.session.execute(text(sql_insert), {"course_name": course_name,
    "course_description":course_description,"teacher_id": teacher_id})
    db.session.commit()

def get_course_by_teacher_id(teacher_id):
    sql_get_courses = """SELECT course_id, course, course_description FROM courses
    WHERE teacher_id = :teacher_id ORDER BY course_id"""
...
def update_course(course_id, course_name, course_description):
    sql_update_course = """UPDATE courses SET course = :course_name,
...
def check_existing_grade(student_id, course_id):
    sql_check_existing_grade = """SELECT COUNT(*) FROM grades WHERE
...
def insert_grade(student_id, course_id, teacher_id, grade):
    sql_insert_grade = """INSERT INTO grades (student_id, course_id,
    teacher_id, grade) VALUES (:student_id, :course_id, :teacher_id, :grade)"""
...
def delete_enrollment(course_id, student_id):
    sql_delete_enrollment = """DELETE FROM enrollments
