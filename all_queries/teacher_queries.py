...
def insert_into_courses(course_name, course_description,teacher_id):
    sql_insert = """
    INSERT INTO courses (course, course_description, teacher_id)
...
def get_course_by_teacher_id(teacher_id):
    sql_get_courses = """
    SELECT course_id, course,course_description
    FROM courses
...
def update_course(course_id, course_name, course_description):
    sql_update_course = """
    UPDATE courses SET course = :course_name,
...
def check_if_course_teacher(course_id, teacher_id):
    sql_check_if_teacher = """
    SELECT COUNT(*) FROM courses
    WHERE course_id = :course_id
    AND teacher_id = :teacher_id"""
    result = db.session.execute(text(sql_check_if_teacher),
        {"course_id":course_id, "teacher_id": teacher_id}).scalar()
    return result > 0